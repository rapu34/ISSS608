...
To explore potential temporal patterns in road fatalities, I analyzed the average number of deaths by day of the week. This visualization helps identify whether weekends or weekdays are more prone to severe accidents.

The chart below shows that Sunday and Saturday have relatively higher average fatalities, suggesting increased risk during weekends—likely due to higher traffic volumes or longer travel distances during leisure periods.
...
This line plot shows how the average number of fatalities varies throughout the day across different days of the week. By incorporating both **hour of day** and **day of week**, the visualization uncovers temporal patterns that may be linked to traffic volume, human behavior, or policy enforcement timing.

We can observe two main peaks in the evening hours (around **18:00 to 21:00**), especially on weekends, which may reflect higher accident risks due to **increased social activity or fatigue**. The early morning hours also show relatively higher fatality rates on certain days.
...
This chart highlights the average number of fatalities per accident across different types of road users. By examining which categories are associated with higher fatality rates, we can better understand vulnerable road user groups and inform safety measures.

The bar chart below shows the **top 10 vehicle types** (including pedestrians) with the highest average fatalities. Notably, **pedestrians** and **bicycle-related incidents** are among the most fatal on average.
...
This section presents a predictive model built using **Poisson regression**, which is suitable for modeling count-based target variables like the number of fatalities. The model is trained using key accident-related factors to estimate expected fatalities under a given scenario.
...
![](images/clipboard-1002224488.png)
